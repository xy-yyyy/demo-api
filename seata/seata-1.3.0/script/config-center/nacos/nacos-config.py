...
for line in open('../config.txt'):
    ...
    if len(sys.argv) == 3:
        ...
        url_postfix = '/nacos/v1/cs/configs?dataId={0}&group=DEFAULT_GROUP&content={1}&tenant={2}'.format(str(pair[0]),str(line[line.index('=')+1:]).strip(),namespace)
    else:
        url_postfix = '/nacos/v1/cs/configs?dataId={}&group=DEFAULT_GROUP&content={}'.format(str(pair[0]),str(line[line.index('=')+1:])).strip()
